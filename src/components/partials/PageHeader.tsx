      ...
      <div className="hidden md:flex items-center space-x-4 xl:space-x-6">
        {PAGE_HEADER_MENUS.map((item) => (
          <Link key={`dsk_${item.href}`} href={item.href}>
            <div className="flex items-center space-x-1 group cursor-pointer hover:text-black">
              <div className="w-5 h-0.5 bg-gray-300 group-hover:bg-black transition-transform duration-700 translate-y-0 group-hover:translate-y-2" />
              <p>{item.name}</p>
            </div>
          </Link>
        ))}
